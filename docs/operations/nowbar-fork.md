...
Enable GitHub Actions and provide `NOWBAR_KEYSTORE_BASE64` (a PKCS12 keystore with alias `nowbar`) and `NOWBAR_KEYSTORE_PASSWORD`. Keep an offline backup of both: future Android updates must use the same signing identity. Never commit signing material. The Android version code is the greater of the workflow run number and the latest published release code plus one, so local releases and scheduled releases can safely share the update channel. GitHub may disable scheduled workflows after prolonged repository inactivity; check Actions if releases stop.

Run the workflow manually with `sync_upstream` to test an upstream merge immediately. To build locally, set `JAVA_HOME` to Java 17, `JAVA21_HOME` to Java 21, and `NOWBAR_VERSION_CODE` to the next unused release code, then run `scripts/build-nowbar-local.sh` under Node 24. The script keeps caches, runs focused tests/types, builds arm64, and signs using the private signing directory (`NOWBAR_SIGNING_DIR` overrides its local default). The display version derives from upstream's mobile version plus `-nowbar.<build number>`. Local debug signing cannot update a published build.
...
The native notification keeps Android's standard contentView and promoted ongoing flag. Samsung receives custom RemoteViews through `android.ongoingActivityNoti.chronometerRemoteView`, its tag and view/Now Bar position extras; using Android `setCustomContentView` instead would disqualify Live Updates. The lab exercises the same component builder used by local and remote notifications. It reports whether the custom payload is attached, separately from whether Samsung actually renders it. Samsung extras are a compatibility adapter, not a promise of an official stable Samsung API. References: [Android Live Updates](https://developer.android.com/develop/ui/views/notifications/live-update), [community Samsung SDK implementation](https://github.com/kirillshsh/nowbar-sdk/blob/main/nowbar/src/main/kotlin/com/nowbar/api/notification/OngoingExtrasBuilder.kt).
